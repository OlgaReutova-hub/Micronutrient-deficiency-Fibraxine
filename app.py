    ...
    if scenario is None:
        ...
        with right_col:
            ...
            if memo_clicked:
                ...
                # На Streamlit (особенно на облачном деплое) сервер не может "открыть" локальный файл
                # в браузере пользователя. Поэтому показываем HTML как модальное окно поверх интерфейса.
                # Используем iframe + data: URL, чтобы можно было отрендерить полный HTML-документ.
                memo_data_url = f"data:text/html;charset=utf-8,{quote(memo_html, safe='')}"
                st.markdown(
                    f"""
                    <div id="patient-memo-modal" class="patient-memo-modal" role="dialog" aria-modal="true">
                      <div class="patient-memo-modal__backdrop"
                           onclick="document.getElementById('patient-memo-modal').style.display='none'"></div>
                      <div class="patient-memo-modal__content">
                        <button class="patient-memo-modal__close"
                                onclick="document.getElementById('patient-memo-modal').style.display='none'">×</button>
                        <iframe class="patient-memo-modal__iframe"
                                src="{memo_data_url}" title="Памятка для пациента"></iframe>
                      </div>
                    </div>
                    <style>
                      .patient-memo-modal {{
                      ...
                      .patient-memo-modal__backdrop {{
                        position: absolute;
                        inset: 0;
                        background: rgba(0,0,0,0.45);
                      }}
                      .patient-memo-modal__content {{
                        position: relative;
                ...
                # На локальном ПК также можно оставить вариант с сохранением/открытием.
                # Ошибки записи/открытия не должны ломать интерфейс.
                try:
                    html_path = Path(__file__).resolve().parent / "data" / MEMO_OUTPUT_NAME
                    html_path.write_text(memo_html, encoding="utf-8")
                    open_html_file_default(html_path)
                except Exception:
                    pass
